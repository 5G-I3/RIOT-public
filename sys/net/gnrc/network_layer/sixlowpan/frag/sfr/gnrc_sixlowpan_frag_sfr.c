...
#define FRAG_DESCS_POOL_SIZE    (CONFIG_GNRC_SIXLOWPAN_FRAG_FB_SIZE * \
                                 GNRC_SIXLOWPAN_SFR_MAX_WIN_SIZE)
...
{
    if (GNRC_SIXLOWPAN_SFR_INTER_FRAME_GAP_US > 0) {
    ...
    if (frag_desc) {
        do {
            ...
            if (diff < (fbuf->sfr.arq_timeout * US_PER_MS)) {
                ...
                if ((frag_desc->retries++) < GNRC_SIXLOWPAN_SFR_FRAG_RETRIES) {
                    DEBUG("6lo sfr: %u retries left for fragment (tag: %u, "
                          "X: %i, seq: %u, frag_size: %u, offset: %u)\n",
                          GNRC_SIXLOWPAN_SFR_FRAG_RETRIES -
...
static void _clean_slate_datagram(gnrc_sixlowpan_frag_fb_t *fbuf)
{
    ...
    fbuf->sfr.window_size = GNRC_SIXLOWPAN_SFR_OPT_WIN_SIZE;
...
static bool _send_frame(gnrc_pktsnip_t *frame, void *ctx, unsigned page)
{
    ...
    if ((GNRC_SIXLOWPAN_SFR_INTER_FRAME_GAP_US == 0) ||
        ((now - _last_frame_sent) > GNRC_SIXLOWPAN_SFR_INTER_FRAME_GAP_US)) {
                                  ...
                                  const uint8_t *bitmap)
{
    ...
    ack = ack_snip->data;
    /* https://tools.ietf.org/html/draft-ietf-6lo-fragment-recovery-07#section-6:
     ...
     * [...] The receiver of a fragment with the
...
{
    sixlowpan_sfr_rfrag_t *hdr = pkt->data;
    /* https://tools.ietf.org/html/draft-ietf-6lo-fragment-recovery-07#section-5.1:
                              ...
                              unsigned page, _generic_rb_entry_t *entry)
{
    ...
    if (gnrc_sixlowpan_frag_rb_exists(netif_hdr, hdr->base.tag)) {
         ...
         * https://tools.ietf.org/html/draft-ietf-6lo-fragment-recovery-07#section-6.1.2 */
                                   ...
                                   unsigned page)
{
    ...
    fbuf->sfr.arq_timeout = GNRC_SIXLOWPAN_SFR_OPT_ARQ_TIMEOUT_MS;
    fbuf->sfr.window_size = GNRC_SIXLOWPAN_SFR_OPT_WIN_SIZE;
...
static void _retry_datagram(gnrc_sixlowpan_frag_fb_t *fbuf)
{
    if ((GNRC_SIXLOWPAN_SFR_DG_RETRIES == 0) ||
        (fbuf->sfr.retrans == 0)) {
...
static void _sched_next_frame(void)
{
    if (GNRC_SIXLOWPAN_SFR_INTER_FRAME_GAP_US > 0) {
        ...
        if (!already_set) {
            ...
            if (last_sent_since <= GNRC_SIXLOWPAN_SFR_INTER_FRAME_GAP_US) {
                uint32_t offset = GNRC_SIXLOWPAN_SFR_INTER_FRAME_GAP_US -
                                  last_sent_since;
