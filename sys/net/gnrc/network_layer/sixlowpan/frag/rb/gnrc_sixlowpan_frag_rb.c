...
static bool _valid_offset(gnrc_pktsnip_t *pkt, size_t offset)
{
    ...
    res = res || (sixlowpan_frag_1_is(pkt->data) && (offset == 0)) ||
                     ...
                     size_t offset, unsigned page)
{
    ...
    if ((interval = gnrc_sixlowpan_frag_rb_int_update(&entry->super, offset,
                                                      frag_size))) {
        ...
        if (IS_USED(MODULE_GNRC_SIXLOWPAN_FRAG_SFR)) {
            ...
            if (gnrc_pktbuf_realloc_data(entry->pkt,
                                         entry->super.datagram_size) != 0) {
                DEBUG("6lo rbuf: can't allocate packet data\n");
                gnrc_pktbuf_release(entry->pkt);
                gnrc_sixlowpan_frag_rb_remove(entry);
                gnrc_pktbuf_release(pkt);
                return RBUF_ADD_ERROR;
